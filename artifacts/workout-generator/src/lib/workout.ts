...
export interface Exercise {
  ...
  needsBench?: boolean;
  // Упражнение специфично для женщин (например, мышцы тазового дна).
  // Для пользователей мужского пола такие упражнения скрываются.
  femaleOnly?: boolean;
}
...
const EXERCISES: Exercise[] = [
  ...
  // --- Тазовое дно (только для женщин) ---
  { name: "Упражнение Кегеля", muscle: "core", equipment: "none", goals: ALL, femaleOnly: true },
  { name: "Ягодичный мостик с акцентом на тазовое дно", muscle: "core", equipment: "none", goals: ALL, femaleOnly: true },
  { name: "Приседание со сведением коленей", muscle: "core", equipment: "none", goals: ALL, femaleOnly: true },
  { name: "«Бабочка» лёжа с подъёмом таза", muscle: "core", equipment: "none", goals: ALL, femaleOnly: true },
  { name: "Диафрагмальное дыхание со сжатием тазового дна", muscle: "core", equipment: "none", goals: ALL, femaleOnly: true },
...
function isAllowed(
  ...
  if (ex.femaleOnly && f.gender !== "female") return false;
...
function cueFor(name: string): string {
  ...
  const rules: Array<[RegExp, string]> = [
    ...
    [/кегел/, "Сожмите мышцы тазового дна (как при остановке мочеиспускания), удерживайте 5–10 сек, расслабьте на столько же. Без задержки дыхания и без напряжения ягодиц/пресса."],
    [/мостик.*тазов.*дн/, "Лёжа на спине, колени согнуты. На выдохе сжимайте тазовое дно и поднимайте таз, удерживайте 3 сек, опускайтесь на вдохе."],
    [/присед.*свед.*колен/, "Стопы на ширине таза, носки чуть наружу. Опускайтесь в неглубокий присед, активно сводя колени друг к другу (изометрическое напряжение приводящих). На подъёме сжимайте тазовое дно. Если есть мяч/подушка/полотенце — можно зажать между коленями для контроля."],
    [/бабочк.*подъём.*таз/, "Лёжа на спине, стопы вместе, колени в стороны. На выдохе поднимайте таз, сжимая тазовое дно."],
    [/диафрагмальн.*дыхан/, "Лёжа на спине, рука на животе. Медленный вдох в живот, на выдохе мягко сжимайте мышцы тазового дна. 8–10 циклов."],
...
function adjustVolumeForAge(base: string, age: number): string {
  if (age >= 65) {
    // 65+ — режим «сеньор» (ACSM Older Adults):
    // снижаем подходы, повторения 8–10, отдых 90 сек, бережём суставы
    return base
  ...
  if (age >= 50) {
    // 50–64 — мягкий взрослый режим (ACSM Middle-Age):
    // повторения 10–12, отдых 90 сек, объём чуть ниже
    return base
      .replace(/^5 подходов/, "3 подхода")
      .replace(/^4 подхода/, "3 подхода")
      .replace(/12–15 повторений/, "10–12 повторений")
      .replace(/8–10 повторений/, "10–12 повторений")
      .replace(/отдых.*30 сек/, "отдых между подходами 90 сек")
      .replace(/отдых.*45 сек/, "отдых между подходами 90 сек")
      .replace(/отдых.*60 сек/, "отдых между подходами 90 сек");
  }
...
function phaseFor(weekIndex0: number, age: number): PhaseModifier {
  const inCycle = weekIndex0 % 4;
  // Для 50+ схема повторений в setsForCoursePhase не варьируется
  // (волна идёт только через рабочий вес — это безопаснее для суставов).
  // Поэтому и описание фазы должно говорить только о весе, без обещаний
  ...
  switch (inCycle) {
    case 0:
      return {
        ...
        description: isMature
          ? "Лёгкая неделя: веса 65% от рабочих, разучиваем технику и готовим связки. Повторения остаются в вашей возрастной схеме."
          : "Лёгкая неделя: разучиваем технику, веса 65% от рабочих, больше повторений (12–15) — мышцы и связки готовятся к нагрузке.",
      };
    case 1:
      return {
        phase: "Рабочая",
        ...
        repsMode: "normal",
        description: isMature
          ? "Базовая нагрузка: 85% от целевых рабочих весов, повторения по вашей возрастной схеме."
          : "Базовая нагрузка: 85% от целевых рабочих весов, схема повторений по уровню подготовки.",
      };
    case 2:
      return {
        phase: "Пиковая",
        ...
        repsMode: "low",
        description: isMature
          ? "Пиковая неделя: рабочие веса 100%, отдых длиннее. Повторения остаются в вашей возрастной схеме — без перехода в силовой диапазон."
    ...
    default:
      return {
        ...
        repsMode: "deload",
        description: isMature
          ? "Восстановление: веса 50%, без отказа — снимаем накопленную усталость перед новым циклом."
          : "Восстановление: 2 подхода, веса 50%, повторения 12–15 без отказа — снимаем накопленную усталость перед новым циклом.",
      };
  }
}
...
export function generateCourse(
  ...
  const generalTips: string[] = [
    ...
    f.age >= 50
      ? "Каждые 4 недели повторяется мезоцикл: втягивающая (65%) → рабочая (85%) → пиковая (100%) → разгрузочная (50%). Для вашей возрастной группы нагрузка от недели к неделе меняется только за счёт рабочего веса — количество повторений остаётся в безопасной возрастной схеме."
