...
export function generateCourse(
  ...
  const generalTips: string[] = [
    ...
    f.age >= 50
      ? "Каждые 4 недели повторяется мезоцикл: втягивающая (65%) → рабочая (85%) → пиковая (100%) → разгрузочная (50%). Для вашей возрастной группы волна реализуется через рабочий вес — повторения остаются в безопасной возрастной схеме."
